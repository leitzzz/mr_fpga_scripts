...
def show_header_message():
    ...
    print("""
             _____                    _____                      _     _ 
            /  ___|                  |  __ \                    (_)   | |
            \ `--.  ___  __ _  __ _  | |  \/ ___ _ __   ___  ___ _ ___| |
            `--. \/ _ \/ _` |/ _` | | | __ / _ \ '_ \ / _ \/ __| / __| |
            /\__/ /  __/ (_| | (_| | | |_\ \  __/ | | |  __/\__ \ \__ \_|
            \____/ \___|\__, |\__,_|  \____/\___|_| |_|\___||___/_|___(_)
                        __/ |                                           
                        |___/                                            
    """)

    print("The main purpose of this script is to download the neccessary files to start to enjoy Sega Genesis on the Mister FPGA")
...
def process_downloads(json_url: str):
    ...
    server_url = json_file_raw_data['d1']
    server_path = json_file_raw_data['dir']

    downloaded_files_count = 1

    for rom_file in json_file_raw_data['files']:
        # only process zip files
        if not rom_file['format'] == 'ZIP':
            continue

        # dont process bios files
        if "BIOS" in rom_file['name']:
            continue
